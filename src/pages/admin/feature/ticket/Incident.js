...
const fetchWaitlist = (page, pageSize, defaultAgency, auth) => {
    ...
    if (auth.role === ROLE_AGENT) {
        url = API_URL + `tickets/waitlist/${auth.username}?page=${page}&pageSize=${pageSize}&type=0&source=`;
    } else {
        url = API_URL + `tickets/waitlist?page=${page}&pageSize=${pageSize}&type=0&source=`;
    }
...
function Incident() {
    ...
    } = useAssignTicket(["waitlist-incident", pagination.current, pagination.pageSize, defaultAgency, auth]);
    ...
    const {
        data: waitlist,
        isLoading
    } = useQuery(["waitlist-incident", pagination.current, pagination.pageSize, defaultAgency, auth], () => fetchWaitlist(pagination.current - 1, pagination.pageSize, defaultAgency, auth), {
        onSuccess: (data) => {
    ...
    const defaultColumns = [
        {
            ...
            dataIndex: 'title',
            key: 'title',
            render: (text, record) => <Link to={`/${GET_ROUTE_WITH_ROLE(auth.role)}/general/incidents/${record.id}`}>{text}</Link>
        },
        {
            title: 'Description',
